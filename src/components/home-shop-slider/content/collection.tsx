...
export default (props:Props) => {
  ...
  const shop = products.map(item => (
    <div key={item.name} className={`col-md-4 col-sm-6 masonry__item filter-${item.filter}`}
      data-masonry-filter={item.filter}>
      <a href="#">
        <div className="shop-item shop-item-1">
          <div className="shop-item__price hover--reveal">
            {!item.last_price?null:<span className="type--strikethrough">{item.last_price}</span>}
      ...
      <div className="container">
        <div className="row">
          <div className="masonry masonry-shop">
            <div className="masonry__container masonry--animate masonry--active" style={{position:"relative",height:"842px"}}>
              {shop}
            </div>
          </div>
        </div>
